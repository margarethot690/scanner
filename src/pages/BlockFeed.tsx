...
export default function BlockFeed() {
  ...
  useEffect(() => {
    if (initialBlocks && blocks.length === 0) {
      ...
      if (firstBlock) {
        lastHeightRef.current = firstBlock.height;
      }
    }
  ...
  const toggleExpand = (height: number): void => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {isLoading
          ? Array.from({ length: 10 }, (_, skeletonIndex) => `skeleton-${skeletonIndex}`).map(
              (skeletonKey) => <Skeleton key={skeletonKey} className="h-32 w-full" />,
            )
          : blocks.map((block) => (
  ...
  if (isLoading) {
    return (
      <div className="mt-3 space-y-2">
        {Array.from({ length: 3 }, (_, skeletonIndex) => `skeleton-${skeletonIndex}`).map(
          (skeletonKey) => (
